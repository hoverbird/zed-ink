...
impl zed::Extension for InkExtension {
    ...
    fn language_server_command(
        ...
        if let Ok(metadata) = fs::metadata(&inklecate_binary.path) {
            if !metadata.is_file() {
                return Err(format!("Binary is not a file: {}", inklecate_binary.path));
            }
        } else {
            return Err(format!("Binary does not exist: {}", inklecate_binary.path));
        }

        // Let's try different arguments for LSP mode
        let args = inklecate_binary.args.unwrap_or_else(|| {
